...
angular.module('routerApp').controller('mainController', ['$scope', '$state', '$http', function ($scope, $state, $http) {
	...
	$scope.cnfrmlogout = function(){
	    $('#confirmModal').modal('hide');
        $state.go('login');
        localStorage.removeItem("orderItems");
    }
	$scope.cart = function(){
	    if(localStorage.getItem("speechPermission") == "true"){
          TTS.speak('Food Cart');
        }
		$state.go('cart');
	}
	...
	$scope.settingsPage = function(){
	    if(localStorage.getItem("speechPermission") == "true"){
              TTS.speak('User Settings');
            }
		$state.go('settings');
	}
	$scope.appsettingsPage = function(){
	    if(localStorage.getItem("speechPermission") == "true"){
              TTS.speak('App Settings');
            }
    ...
    vm.startRecognition = function () {
        window.plugins.speechRecognition.startListening(function(result){
                  ...
                  if(result[0] == "menu card"){
                    $state.go('menu');
                  }if(result[0] == "order cart"){
                    $state.go('cart');
                  }if(result[0] == "food card"){
                    $state.go('card');
                  }if(result[0] == "flyers"){
                    $state.go('flyer');
                  }if(result[0] == "user settings"){
                    $state.go('settings');
                  }if(result[0] == "app settings"){
                    $state.go('appSettings');
                  }
              }, function(err){
                  alert(err);
...
angular.module('routerApp').controller('appSettingsController', ['$scope', '$state', '$http', function ($scope, $state, $http) {
    ...
    if(localStorage.getItem("speechPermission") == "false"){
      document.getElementById("speechPermission").checked = false;
    }else{
      document.getElementById("speechPermission").checked = true;
    }
    ...
    $scope.enableSpeech = function(){
          if($("#speechPermission").is(':checked') == true){
            localStorage.setItem("speechPermission", true);
          }else{
            localStorage.setItem("speechPermission", false);
          }
        }
    $scope.nativeSettings = function(){
        if (window.cordova && window.cordova.plugins.settings) {
            console.log('openNativeSettingsTest is active');
            window.cordova.plugins.settings.open("accessibility", function() {
